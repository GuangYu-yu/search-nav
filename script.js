...
document.addEventListener('DOMContentLoaded', function() {
    ...
    searchInput.addEventListener('input', (e) => {
        ...
        if (currentMode === 'search') {
            fetchSuggestions(query).then(suggestions => showSuggestions(suggestions, query));
        } else {
            hideSuggestions();
        }
    });
    ...
    searchInput.addEventListener('keypress', (e) => {
        if (e.key === 'Enter' && !document.getElementById('suggestionsContainer').classList.contains('show')) {
            handleSearch();
        }
    });
    ...
    document.getElementById('engineDropdown').addEventListener('mousedown', (e) => {
        e.preventDefault();
    });
...
const SUGGESTION_APIS = {
    ...
    so360: {
        ...
        params: { word: '', encodein: 'utf-8', encodeout: 'utf-8', callback: 'callback' }
    }
};
...
function performRequest(engine, api, query) {
    ...
    if (engine === 'duckduckgo') {
        return fetch(`${api.url}?${new URLSearchParams({ ...api.params, q: query })}`)
    ...
    return new Promise((resolve, reject) => {
        ...
        window[callbackName] = function(data) {
            ...
            try {
                ...
                if (engine === 'so360') {
                    ...
                    suggestions = data.result ? data.result.map(item => item.word || item) : [];
                }
        ...
        const params = new URLSearchParams({ 
            ...api.params, 
            [Object.keys(api.params).find(key => key === 'q' || key === 'query' || key === 'command' || key === 'word')]: query,
            [Object.keys(api.params).find(key => key === 'callback' || key === 'JsonpCallback')]: callbackName
        });
...
async function fetchSuggestions(query) {
    ...
    // 获取当前选中的搜索引擎
    const currentEngine = document.querySelector('.engine-icon').className
        .split(' ') 
